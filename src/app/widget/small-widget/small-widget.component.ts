...
export class SmallWidgetComponent implements OnInit {
    ...
    this.form.forEach((formField: any) => {
      if (formField["primary"]) {
        switch (formField["type"]) {
          case "phone":
            formGroupConfig[formField['name']] = ['', [Validators.required, Validators.pattern("^[0-9]*$"), Validators.minLength(10), Validators.maxLength(10)]];
            break;
          case "email":
            formGroupConfig[formField['name']] = ['', [Validators.required, Validators.email]];
            break;
          default:
  ...
  saveForm = (): void => {
    ...
    userInputValues["id"] = userInputValues["formId"] + '' + userInputValues["tenantId"];
    ...
    this.ws.saveUserForm(userInputValues).subscribe(data => {
      if (data["status"] == "ok") {
        ...
        this.errorMessage = "Data has not saved on the server. Technical Error: " + data["data"];
      }
    });
  }
}